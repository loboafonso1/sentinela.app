...
import { Volume2, Bell, Shield, Lock, ChevronRight } from "lucide-react";
  ...
  const sections = [
    {
      title: "Preferências",
      items: [
        { label: "Efeitos Sonoros", icon: Volume2, type: "switch", state: audioEnabled, setter: setAudioEnabled },
        { label: "Notificações Push", icon: Bell, type: "switch", state: notifEnabled, setter: setNotifEnabled },
      ]
    },
    {
      title: "Segurança",
      items: [
        { label: "Privacidade da Conta", icon: Shield, type: "link" },
        { label: "Termos de Uso", icon: Lock, type: "link" },
      ]
    }
  ];
      ...
      <main className="relative z-10 px-6 space-y-10">
        {sections.map((section, i) => (
          <motion.div
            ...
            className="space-y-4"
          >
            <h4 className="text-[10px] uppercase tracking-[0.5em] text-[#7A00FF] font-bold px-2">
              {section.title}
            </h4>
            ...
            <div className="space-y-2">
              {section.items.map((item) => (
                <div
                  key={item.label}
                  className="w-full flex items-center justify-between p-6 bg-white/[0.02] border border-white/5 rounded-[2rem]"
                >
