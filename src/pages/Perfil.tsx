...
import { Award, ChevronRight, LogOut, Shield, User } from "lucide-react";
import BottomNav from "@/components/BottomNav";
import { useAuth } from "@/hooks/useAuth";

const Perfil = () => {
  const { user, signOut } = useAuth();
  const [levelName, setLevelName] = useState("Nível não definido");
  ...
  return (
    <div className="min-h-screen bg-[#0A0014] text-white pb-32">
      ...
      <header className="relative z-10 px-8 pt-12 pb-12 flex flex-col items-center">
        ...
        <motion.h1 initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="text-2xl font-bold tracking-tight mb-1">
